...
//        if let myHealthStore = healthStore {
//            let heartRateType = HKQuantityType.quantityTypeForIdentifier(HKQuantityTypeIdentifierHeartRate)
//            let updateHandler: ((HKObserverQuery!, HKObserverQueryCompletionHandler!, NSError!) -> Void) = { (query: HKObserverQuery!, handler: HKObserverQueryCompletionHandler!, error: NSError!) in
//                println("updateHandler thread = \(NSThread.currentThread()) queue = \(dispatch_get_current_queue())")
//                if (error) {
//                    println("observer query returned error = \(error)")
//                } else {
//                    let endDate = NSSortDescriptor(key: HKSampleSortIdentifierEndDate, ascending: false)
//                    let sortDescriptors = NSArray(object: endDate)
//                    let resultsHandler: ((HKSampleQuery!, AnyObject[]!, NSError!) -> Void) = { (query: HKSampleQuery!, object: AnyObject[]!, error: NSError!) in
//                        if (error) {
//                            println("sample query returned error = \(error)")
//                        } else {
//                            println("got a value")
...
//                                        let value = quantity.doubleValueForUnit(self.heartRateUnit)
//                                        self.bpmLabel.text = String(Int(value))
...
//                                }
//                            } else {
//                                println("not count in results array")
//                            }
//                        }
//                    }
//                    println("got an update")
//                    
//                    let sampleQuery = HKSampleQuery(sampleType: heartRateType, predicate: nil, limit: 1, sortDescriptors: sortDescriptors, resultsHandler: resultsHandler)
//                    myHealthStore.executeQuery(sampleQuery)
//                }
//            }
...
//                    let endDate = NSSortDescriptor(key: HKSampleSortIdentifierEndDate, ascending: false)
//                    let sortDescriptors = NSArray(object: endDate)
//                    let sampleQuery = HKSampleQuery(sampleType: heartRateType, predicate: nil, limit: 1, sortDescriptors: sortDescriptors) {
//                        (query: HKSampleQuery!, object: AnyObject[]!, error: NSError!) in
//                        if (error) {
//                            println("sample query returned error = \(error)")
//                        } else {
